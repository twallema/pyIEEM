...
data_epi_BE = get_hospitalisation_incidence(
    'BE', aggregate_bxl_brabant=True).loc[slice(start_calibration, end_calibration)]
data_epi_SWE = get_hospitalisation_incidence(
...
model_BE = initialize_epinomic_model('BE', age_classes, True, start_calibration)
model_SWE = initialize_epinomic_model('SWE', age_classes, True, start_calibration)
...
log_likelihood_fnc_args = [len(data_epi_BE.index.get_level_values('spatial_unit').unique())*[alpha,],
                           len(data_epi_SWE.index.get_level_values('spatial_unit').unique())*[alpha,],
                           0.04,
                           0.04]
